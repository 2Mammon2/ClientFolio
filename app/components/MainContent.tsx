...
function MainContent() {
  ...
  // Quick links (pentest, soc) — top section
  const quickLinksPosts = data
    .filter(
      (item) =>
  ...
  // Recent posts — exclude pentest, soc
  const postsForRecent = data.filter(
    (item) =>
      Array.isArray(item.category) &&
      item.category.every(
        (cat) =>
  ...
  const visiblePosts = postsForRecent.slice(start, end);
  const totalPages = Math.ceil(postsForRecent.length / POSTS_PER_PAGE);
